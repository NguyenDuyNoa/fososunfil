...
const BannerProduct = ({ dataCategory, dataProduct }: BannerProductProps) => {
  ...
  return (
    <div className="rounded-xl bg-brand-600 overflow-hidden">
      <div
        className="relative h-[120px] xl:h-[500px] overflow-hidden w-full flex justify-center items-center bg-cover bg-center bg-no-repeat"
        style={{ backgroundImage: `url(${dataCategory?.background})` }}
      >
        ...
        <div className="absolute top-[60%] xl:top-1/2 -translate-y-1/2 left-2 xl:left-10 flex flex-col gap-1 xl:gap-5 max-w-[60%]">
          <div className="relative inline-block text-[20px]/[21px] xl:text-[80px] font-extrabold uppercase tracking-tighter select-none">
            ...
            <h2 className="relative text-white">{dataCategory?.title}</h2>
          </div>
          ...
          {/* Tích điểm ngay trên app{" "}
            ...
            *100K = 10 điểm */}
          <div
            className="text-[8px]/[12px] xl:text-[32px]/[48px] font-medium text-warning-light"
            dangerouslySetInnerHTML={{ __html: dataCategory?.sub_title }}
          ></div>
          {/* </p> */}
        </div>
