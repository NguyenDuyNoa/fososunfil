...
import { Brand, Category, Origin } from "@/types/products/IProducts";
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="absolute inset-0 sm:inset-auto sm:right-0 sm:top-0 sm:bottom-0 w-full sm:w-96 bg-white flex flex-col overflow-hidden z-10">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {/* Bộ lọc sản phẩm */}
          {mockFilterData?.productFilters && mockFilterData.productFilters.length > 0 && (
            <FilterSection title="Bộ lọc sản phẩm" isOpen={true}>
              <div className="space-y-2">
                {mockFilterData.productFilters.map((filter: CategoryFilter) => {
          ...
          {additionalFilterData && (
            <FilterSection title="Dòng xe" isOpen={true}>
              <div className="space-y-1">
                {Object.entries(additionalFilterData).map(([key, filterGroup]) => (
                  <FilterNestedSection key={key} title={filterGroup.label} isOpen={true}>
